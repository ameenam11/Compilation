 ...
 module Tag_Parser : TAG_PARSER = struct
                 ...
                 ScmPair (expr,
                          ScmPair (dit,
                                   ScmPair (ScmBoolean false,
                                            ScmNil))));;
   let rec macro_expand_let expr = function
   | [] -> expr
   | expr' :: exprs ->
     let dit = macro_expand_let expr' exprs in
   ...
   let names_and_values = 
     let rec run = function
       ...
       | _ -> raise (X_syntax "wrong syntax")
       in run;;
 
   let pairs_of_list s e = 
     List.fold_right(fun car cdr -> ScmPair (car, cdr)) s e;;
   ...
   let rec tag_parse sexpr =
     ...
     (* add support for if *)
       (*Mohammad*)
     | ScmPair (ScmSymbol "if", ScmPair (expr1, ScmPair (expr2, ScmPair(expr3, ScmNil)))) ->
         ScmIf (tag_parse (expr1), tag_parse (expr2), tag_parse (expr3))
     | ScmPair (ScmSymbol "if", ScmPair (expr1, ScmPair (expr2, ScmNil))) ->
       ScmIf (tag_parse (expr1), tag_parse (expr2), tag_parse(ScmVoid))
       (*Mohammad*)
     ...
     | ScmPair (ScmSymbol "define",ScmPair (ScmSymbol var,ScmPair (expr, ScmNil))) ->     
       if (is_reserved_word var)
       then raise (X_syntax "you can't define a reserved word")
       else ScmVarDef(Var var, tag_parse expr)
     | ScmPair (ScmSymbol "define",ScmPair (ScmPair (var, params),exprs)) ->
         let var_as_function = ScmPair (ScmPair (ScmSymbol "lambda", ScmPair (params, exprs)), ScmNil) in
         tag_parse (ScmPair (ScmSymbol "define", ScmPair (var, var_as_function)))                 
     (*Mohammad*)
     ...
     | ScmPair (ScmSymbol "let*", ScmPair (ScmPair (one_rib, ScmNil), exprs)) ->
         let let_ribs = ScmPair (one_rib, ScmNil) in
         tag_parse (ScmPair (ScmSymbol "let", ScmPair (let_ribs, exprs)))
       
     | ScmPair (ScmSymbol "let*", ScmPair (ScmPair (ScmPair (param, arg), rest_ribs), exprs)) ->                                                                                     
         let one_rib = ScmPair (param, arg)in
         let let_ribs = ScmPair (one_rib, ScmNil) in
         let let_body = ScmPair (ScmPair (ScmSymbol "let*", ScmPair (rest_ribs, exprs)), ScmNil) in                   
         tag_parse (ScmPair (ScmSymbol "let", ScmPair (let_ribs, let_body)))                 
     (*Mohammad*)
     ...
     | ScmPair (ScmSymbol "letrec", ScmPair (ribs, exprs)) ->
       ...
       let dummy_bindings =
         List.map
           (fun v ->
             ScmPair (v, ScmPair (ScmPair ( ScmSymbol "quote", ScmPair (ScmSymbol "whatever", ScmNil)), ScmNil)))
           names in
       ...
       tag_parse (ScmPair (ScmSymbol "let", ScmPair (dummy_ribs, new_exprs)))
     (*Mohammad*)
 ...
 module Semantic_Analysis : SEMANTIC_ANALYSIS = struct
   ...
   let annotate_tail_calls = 
     let rec run in_tail = function
       ...
       (* add support for if *)
       (* Mohammad *)
       | ScmIf' (expr1, expr2, expr3) -> ScmIf'(run false expr1, run in_tail expr2, run in_tail expr3)
       (* Mohammad *)
       (* add support for sequences *)
       | ScmSeq'([]) ->ScmSeq' [] 
       | ScmSeq'(exprs) -> 
         let first_expr = List.hd exprs in
         let rest_exprs = List.tl exprs in
         ScmSeq' (runl in_tail first_expr rest_exprs)
       (* add support for or *)
       (* Mohammad *)
       | ScmOr'([]) ->ScmOr' [] 
       | ScmOr'(exprs) -> 
         let first_expr = List.hd exprs in
         let rest_exprs = List.tl exprs in
         ScmOr' (runl in_tail first_expr rest_exprs)
       (* Mohammad *)
       ...
       (* Mohammad *)
       | ScmApplic' (proc, params, kind) -> 
         if in_tail
           then ScmApplic' (run false proc , (List.map (fun param -> run false param) params), Tail_Call)
           else ScmApplic' (run false proc , (List.map (fun param -> run false param) params), Non_Tail_Call)
       (* Mohammad *)
        ...
        module Code_Generation (* : CODE_GENERATION *) = struct
          ...
          let collect_constants =
           (*final project1*)
            let rec run expr' = match expr' with 
               |ScmConst' (ScmVoid) -> [ScmVoid]
               |ScmConst' (ScmSymbol s) -> [(ScmSymbol s)]
               |ScmConst' (number) -> [number]
               |ScmVarGet' (Var' (var, _)) -> [ScmString var]
               |ScmApplic' (var,
                           exprs,
                           kind) -> List.concat([run var] @ (List.map run exprs))
               |ScmLambda'(params, kind, exprs) -> List.concat[run exprs]
               |ScmSeq'(exprs) -> (List.concat(List.map run exprs))
               |ScmOr'(exprs) -> (runs exprs) (*another way*)
               |ScmIf' (test_expr', then_expr', else_expr') -> List.concat([run test_expr'] @ [run then_expr'] @ [run else_expr'])
               |ScmVarSet'(Var' (var, _), expr') -> [ScmString var] @ (run expr') (*im not sure if here we should do var*)
               |ScmVarDef'(Var' (var, _), expr') -> [ScmString var] @ (run expr')
               |ScmBoxSet'(var, expr') -> run expr'
               |ScmBox'(Var' (var, _)) ->[ScmString var]
          ...
          let const_repr sexpr loc table = match sexpr with
            ...
            | ScmString str ->
               ...
               ([RTTI "T_string"; Quad (str_length); ASCII str], 1 + word_size + str_length) 
            | ScmSymbol sym ->
            ...
            | ScmNumber (ScmFraction (numerator, denominator)) ->
               (*final project3*)
               ([RTTI "T_fraction"; Quad numerator; Quad denominator], 1 + 2 * word_size) (*according to TA*)
            | ScmNumber (ScmReal x) ->
               ([RTTI "T_real"; QuadFloat x], 1 + word_size)
            | ScmVector s ->
               (*final project4*)
               let list_size = (List.length s)in
               let my_list = (List.map 
                                   (fun sexpr -> (search_constant_address (sexpr) table))
                                   s)in
               
               ([RTTI "T_vector"; Quad list_size] @ (List.map 
                                                           (fun addr -> ConstPtr addr)
                                                           my_list),1 + word_size + 8*list_size) (*according to TA*)
            | ScmPair (car, cdr) ->
               (*final project5*)
               let car_addr = search_constant_address (car) table in
               let cdr_addr = search_constant_address (cdr) table in
               ([RTTI "T_pair"; ConstPtr car_addr; ConstPtr cdr_addr], 1 + 2 * word_size);; (*according to TA*)
          ...
          let collect_free_vars =
           (*final project6*)
            let rec run = function
             ...
             |ScmApplic' (var,
                         exprs,
                         kind) -> (run var) @ (runs exprs)
             
             |ScmLambda' (strs,
                         kind,
                         body) -> List.concat([run body])
             |ScmVarDef'(Var' (var, Free), expr') ->  [var] @ (run expr')
             |ScmVarSet'(Var' (var, Free), expr') -> [var] @ (run expr')
             |ScmVarSet' (Var' (var, Bound(_,_)), expr') -> (run expr')
             |ScmBoxSet'(var, expr') -> run expr'
             |ScmBox'(Var' (var, _)) -> [var]
             |ScmIf'(test_expr', then_expr', else_expr') -> List.concat([run test_expr'] @ [run then_expr'] @ [run else_expr'])
          ...
          let code_gen exprs' =
            ...
            let rec run params env = function
              ...
              | ScmOr' exprs' ->
                (*final project7*)
                let label_or = make_or_end () in
                let separate_list = list_and_last exprs' in
                let sub_exprs = (match separate_list with 
                                |None -> []
                                |Some (car, cdr) -> car) in
                let last = (match separate_list with
                                
                                |Some (car, cdr) -> cdr) in
            
                String.concat "\n" 
                (List.map (fun one_expr' ->
                  (run params env one_expr') 
              ...
              | ScmVarSet' (Var' (v, Free), expr') ->
                 (*final project8*)               
                 let label = search_free_var_table v free_vars in
                 let eval = run params env expr' in
                 eval
              ...
              | ScmBoxGet' var' ->
                 ...
                 ^ "\tmov rax, qword [rax]\n"
              | ScmBoxSet' (var', expr') -> (*we must test this*)
                 (*final project12*)
                 let eval = run params env expr' in
                 eval
                 ^"\tpush rax\n"
              ...
              | ScmLambda' (params', Opt opt, body) ->
                  (*final project13*)
                  let lambda_opt_loop_env = make_lambda_opt_loop_env () 
                  and lambda_opt_loop_env_end = make_lambda_opt_loop_env_end ()
                  and lambda_opt_loop_params = make_lambda_opt_loop_params ()
                  and lambda_opt_loop_params_end = make_lambda_opt_loop_params_end ()
                  and lambda_opt_code = make_lambda_opt_code ()
                  and lambda_opt_end = make_lambda_opt_end ()
                  ...
                  and lambda_opt_loop = make_lambda_opt_loop ()
                  and lambda_opt_loo2 = make_lambda_opt_loop ()
                  and lambda_opt_loop_exit = make_lambda_opt_loop_exit ()
                  and lambda_opt_loop_exit2 = make_lambda_opt_loop_exit ()
                  in                                            
                  "\tmov rdi, (1 + 8 + 8)\t; sob closure\n"
                 ^ "\tcall malloc\n"
                 ^ "\tpush rax\n"
                 ...
                 ^ "\tpop rbx\n"
                 ^ "\tmov rsi, 0\n"
                 ^ (Printf.sprintf "%s:\t; copy params\n" lambda_opt_loop_params)
                 ^ (Printf.sprintf "\tcmp rsi, %d\n" params)
                 ^ (Printf.sprintf "\tje %s\n" lambda_opt_loop_params_end)
                 ^ "\tmov rdx, qword [rbp + 8 * rsi + 8 * 4]\n"
                 ^ "\tmov qword [rbx + 8 * rsi], rdx\n"
                 ^ "\tinc rsi\n"
                 ^ (Printf.sprintf "\tjmp %s\n" lambda_opt_loop_params)
                ...
                (* starting the opt *)
                ^ (Printf.sprintf "\t%s:\n" lambda_opt_code)
                ^ (Printf.sprintf "\tmov rcx, qword [rsp + 8 * 2]\n" )
                ^ (Printf.sprintf "\tcmp rcx, %d\n" (List.length params')) 
                ^ (Printf.sprintf "\tjl L_error_incorrect_arity_opt\n" )
                ...
                (* start the Lup*)
                ^ (Printf.sprintf "\t%s:\n" lambda_opt_arity_more)
                ^ "\tlea r9, [rsp + 8*2 + 8*rcx]\n"
                ^  "\tmov r8, qword[rsp + 2*8]\n"
                ^  "\tlea rdx, [rsp + 8*2 + 8*r8]\n"
                ^ (Printf.sprintf "\tlea rsi, [rsp + 8*2 + 8*%d]\n" (List.length params'))
                ^  "\tmov r10, sob_nil\n"
                ^ (Printf.sprintf "\tsub rcx, %d\n" (List.length params'))
                (* L1 *)
                ^ (Printf.sprintf "\t%s:\n" lambda_opt_loop)
                ^ "\tcmp rcx, 0\n"
                ^ (Printf.sprintf "\tje %s\n" lambda_opt_loop_exit)
                ^ "\tmov rdi, (1 + 8 + 8)\n"
                ^ "\tcall malloc\n"
                ^ "\tmov byte[rax], T_pair\n"
                ^ "\tmov rbx, qword[r9]\n"
                    ...
                    String.concat ""
                      (List.map
                         (fun arg ->
                           ...
                           ^ "\tpush rax\n")
                         (List.rev args)) in
                  let proc_code = run params env proc 
                  in
